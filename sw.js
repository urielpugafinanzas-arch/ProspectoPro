// ── La versión del cache se sincroniza automáticamente desde index.html ──
// NO edites CACHE_NAME aquí — solo cambia APP_VERSION en index.html
let CACHE_NAME = 'prospectopro-v1.2.0'; // valor inicial, se sobreescribe vía postMessage
...
self.addEventListener('message', e => {
  // Sincronización automática de versión — recibida desde index.html
  if (e.data && e.data.type === 'SET_VERSION') {
    const newCache = 'prospectopro-v' + e.data.version;
    if (newCache !== CACHE_NAME) {
      const oldCache = CACHE_NAME;
      CACHE_NAME = newCache;
      // Eliminar cache viejo y crear el nuevo con los assets
...
function checkScheduledNotifs(prospects, agenda) {
  ...
  // Citas de agenda — avisar con rango ±1 min para no depender del minuto exacto
  agenda.forEach(cita => {
    ...
    if (diff >= 29 && diff <= 31) {
      self.registration.showNotification('🔔 Cita en 30 minutos', {
        body: `${cita.hora} · ${cita.prospectName || 'Cita agendada'}`,
        icon: './icon-192.png',
        badge: './icon-192.png',
